...
const Header: React.FC<HeaderProps> = ({ activeSection, onHelpClick, onMenuClick, onExport }) => {
  const getTitle = () => {
    switch (activeSection) {
      case StrategySection.MARKET: return 'Competitive Analysis & Positioning';
      ...
      case StrategySection.ADVISOR: return 'AI Strategic Advisor';
      case StrategySection.VAULT: return 'Strategic Memory Vault';
  ...
  const getSubtitle = () => {
    switch (activeSection) {
      case StrategySection.MARKET: return 'Defining the competitive edge and target persona friction.';
      ...
      case StrategySection.ADVISOR: return 'Consult your AI-powered strategic partner for real-time pivots.';
      case StrategySection.VAULT: return 'Retained insights and high-value strategic captures.';
      default: return '';
    }
  };
  ...
  return (
    <header id="main-header" className="p-6 md:p-8 border-b border-slate-800 glass-panel flex justify-between items-center sticky top-0 z-40">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl md:text-2xl font-black text-white tracking-tighter leading-tight uppercase">{getTitle()}</h2>
          <p className="text-slate-400 text-[10px] md:text-xs mt-1 hidden md:block uppercase tracking-widest font-bold">{getSubtitle()}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <button 
          onClick={onHelpClick}
          className="p-2.5 rounded-xl bg-slate-800/50 hover:bg-slate-800 text-slate-400 hover:text-white transition-all border border-slate-700"
          title="Restart Tour"
        >
        ...
        <button 
          onClick={onExport}
          className="hidden md:block px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-widest transition-all spark-glow"
        >
          Export Deck
        </button>
      </div>
